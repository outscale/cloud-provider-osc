...
func (c *Cloud) LoadBalancerExists(ctx context.Context, l *LoadBalancer) (bool, error) {
	...
	svcName := getServiceNameFromTags(lb.Tags)
...
func (c *Cloud) CreateLoadBalancer(ctx context.Context, l *LoadBalancer, backend []VM) (dns string, ip *string, err error) {
	...
	case res.DnsName == "":
		return "", nil, ErrLoadBalancerIsNotReady
	default:
		return res.DnsName, res.PublicIp, nil
	}
}
...
func (c *Cloud) ensureSecurityGroup(ctx context.Context, l *LoadBalancer) error {
	...
	case getClusterIDFromTags(sg.Tags) == "": // new SG or existing SG without tag => create
...
func (c *Cloud) getSecurityGroups(ctx context.Context, l *LoadBalancer, vms []VM, existing *osc.LoadBalancer) error {
	...
	if l.targetSecurityGroup == nil {
		...
		for _, sg := range sgs {
			if hasTag(sg.Tags, c.mainSGTagKey()) {
		...
		if l.targetSecurityGroup == nil {
			klog.FromContext(ctx).V(3).Info("No security group found by tag, using a random one", "securityGroupId", sgs[0].SecurityGroupId)
